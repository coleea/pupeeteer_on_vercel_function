...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  // console.debug('🐞process.env.ENV');
  // console.debug(process.env.ENV);
  // console.debug("process.env");
  // console.debug(process.env.VERCEL_REGION?.includes('dev'));

  console.debug('🐞process.env.VERCEL_REGION');
  console.debug(process.env.VERCEL_REGION);
  //  process.env.ENV === 'local'
      // ? playwright.chromium.executablePath()

console.debug('🐞await chrome.executablePath()');
console.debug(await chrome.executablePath());
  ...
  const browser = await puppeteer.launch(
    ...
    // https://github.com/Sparticuz/chromium/releases/download/v137.0.1/chromium-v137.0.1-pack.x64.tar
      ? {
          ...
          executablePath:  'C:\Users\USER\AppData\Local\Temp\chrome2\chrome.exe',
          // await chrome.executablePath(),
          // process.env.VERCEL_REGION?.includes('dev') ? "/chromium" :
          // await chrome.executablePath(),
          headless: false,
